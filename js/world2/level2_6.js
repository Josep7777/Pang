class level2_6 extends Phaser.Scene {
  constructor() {
    super({ key: "level2_6" });
  }
  ...
  create() {
    ...
    this.loadAnimations();
    this.add.sprite(config.width / 2, config.height / 2 - 100, "background6");
    //Cargamos las pools
    this.loadPools();
    ...
    this.createBall(config.width / 2, config.height / 2 - 270, 4, 1);
    this.createBall(config.width / 2, config.height / 2 - 270, 4, -1);
  ...
  createPlatforms() {
    ...
    var _platform3 = this.add
      .sprite(config.width / 2, config.height / 2 - 150, "destructPlat")
      .setScale(0.4);
