class level2_4 extends Phaser.Scene {
  constructor() {
    super({ key: "level2_4" });
  }
  ...
  create() {
    this.sound.stopAll();
    this.backgroundMusic = this.sound.add("mtKeirin", {
      loop: false,
      volume: 0.5,
    });
    ...
    this.add.sprite(config.width / 2, config.height / 2 - 82, "background4");
    //Cargamos las pools
    this.loadPools();
    ...
    //Datos del HUD
    this.levelName = gamePrefs.WORLD2_NAME;
    this.worldNumber = 2;
    this.stageNumber = 4;
  ...
  createPlatforms() {
    ...
    .sprite(config.width / 2 - 50, config.height - 380, "normalBlueHorizontal")
    .setScale(0.25,0.6);
  ...
  .sprite(config.width / 2 + 90, config.height - 380, "normalBlueHorizontal")
  .setScale(0.25,0.6);
